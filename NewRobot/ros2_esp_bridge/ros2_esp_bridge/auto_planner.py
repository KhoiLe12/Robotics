...
class AutoLandmarkMapCreator(Node):
    ...
    def visualize_partial_maps(self):
        """Visualize all recorded partial maps to see what LiDAR actually detected"""
        ...
        for i, map_data in enumerate(self.individual_maps):
            ...
            # Add robot position
            ax.plot(avg_pos[0], avg_pos[1], 'ro', markersize=10, label='Robot Position')
            ax.legend()
        ...
        for i, map_data in enumerate(self.individual_maps):
            prob_grid = map_data['probability_grid']
            occupied_cells = np.sum(prob_grid > 0.3)
            total_cells = prob_grid.size
            coverage = (occupied_cells / total_cells) * 100
            print(f"  {map_data['name']}: {occupied_cells}/{total_cells} cells ({coverage:.1f}% coverage)")
