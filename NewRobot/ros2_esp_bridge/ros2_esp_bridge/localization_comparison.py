...
class LocalizationComparison(RectanglePathController):
    ...
    def parse_telemetry(self, line):
        """Parse telemetry line: T,<ms>,<L>,<R>,<x>,<y>,<theta>"""
        try:
            parts = line.split(',')
            if len(parts) == 7:
    ...
    def log_corner_with_all_methods(self, corner_num, width, height):
        ...
        # Method 1: Dead Reckoning (now from ESP telemetry)
        pos_data = self.get_current_position()
        if pos_data:
            dr_pos = (pos_data['x'], pos_data['y'])
            print(f"   Dead Reckoning (ESP): ({dr_pos[0]:.3f}, {dr_pos[1]:.3f})m")
        else:
            dr_pos = (0.0, 0.0)
            print(f"   Dead Reckoning (ESP): (0.000, 0.000)m")
        # Method 2: Landmark-Based (unchanged)
        print("Getting landmark position...")
        if corner_num == 0:
            lm_pos = (0.0, 0.0)
            print("   Landmark-Based: Starting position (0.0, 0.0)m")
        else:
            rclpy.spin_once(self.landmark_tracker, timeout_sec=1.0)
    ...
    # Remove calculate_dead_reckoning_position and _reset_dead_reckoning methods
    # In plot_comparison and accuracy calculations, use self.dead_reckoning_positions (from ESP) as before
