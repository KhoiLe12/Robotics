...
# LiDAR yaw offset (radians): +1.5708 if LiDAR faces left of robot front
...
class LocalizationComparison(RectanglePathController):
    def __init__(self, ros2_mode=True):
        import rclpy
        rclpy.init()
        node = rclpy.create_node('localization_comparison')
        super().__init__(ros2_mode=ros2_mode)
        self.ros2_node = node
        ...
        self.odom_sub = self.ros2_node.create_subscription(
            Odometry,
            '/odom',
            self.odom_callback,
            odom_qos
        )
        # Start ROS2 executor and spinning in background thread
        from rclpy.executors import SingleThreadedExecutor
        self.executor = SingleThreadedExecutor()
        self.executor.add_node(self.ros2_node)
        self.executor.add_node(self.landmark_tracker)
        import threading
    ...
    def _spin_ros(self):
        """Background thread to spin ROS2 executor for both nodes"""
        import rclpy
    ...
    def calculate_dead_reckoning_position(self):
        """Get position from latest ROS2 Odometry message (dead reckoning)"""
    ...
    def cleanup(self):
        """Clean up resources"""
        # Only close serial if not in ROS2 mode
        if hasattr(self, 'ros2_mode') and not self.ros2_mode:
            self.close()  # Close serial connection
        # Stop ROS2 spinning
        self.ros_shutdown.set()
        if hasattr(self, 'ros_thread'):
            self.ros_thread.join(timeout=1.0)
        # Remove nodes from executor and shutdown
        import rclpy
        if hasattr(self, 'executor'):
            self.executor.remove_node(self.ros2_node)
            self.executor.remove_node(self.landmark_tracker)
        if rclpy.ok():
