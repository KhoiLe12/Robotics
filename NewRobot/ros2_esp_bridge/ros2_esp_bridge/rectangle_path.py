...
class RectanglePathController:
    ...
    def log_corner_position(self, corner_num):
        ...
        if pos_data:
            corner_info = {
                ...
                'theta': pos_data.get('theta', None),
                'time': datetime.now().isoformat()
            }
            self.corner_positions.append(corner_info)
            print(f"📍 Corner {corner_num} logged: L={pos_data['left_ticks']}, R={pos_data['right_ticks']}, x={pos_data.get('x', None):.4f}, y={pos_data.get('y', None):.4f}, theta={pos_data.get('theta', None):.4f}")
    ...
    def plot_rectangle_path(self, width, height):
        ...
        # Use x, y from ESP telemetry for actual positions
        x_positions = []
        y_positions = []
        for corner in self.corner_positions:
            if corner.get('x') is not None and corner.get('y') is not None:
                x_positions.append(corner['x'])
                y_positions.append(corner['y'])
            else:
